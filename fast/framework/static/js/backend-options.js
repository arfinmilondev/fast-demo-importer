...
var fwBackendOptions = {
	/**
	 * @deprecated Tabs are lazy loaded https://github.com/ThemeFuse/Fast/issues/1174
	 */
		...
		function addPostboxToggles($boxes) {
			...
			$boxes.find('.postbox-header .hndle, .postbox-header .handlediv').on( 'click', function( e ) {
					...
					if ($box.parent().is('.fw-backend-postboxes') && !$box.siblings().length) {
						// Do not close if only one box https://github.com/ThemeFuse/Fast/issues/1094
	...
	(function(){
		var htmlAttrName = 'data-fw-tab-html',
			...
			initAllTabs = function ($el) {
				...
				// fixes https://github.com/ThemeFuse/Fast/issues/1634
				$el.each(function(){
					if ($(this).is(selector)) {
						initTab($(this));
					}
