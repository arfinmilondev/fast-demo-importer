...
final class _FW_Component_Backend {
	...
	public function render_about_page() {
		...
		$file = WP_PLUGIN_DIR . '/fast/framework/views/about.php';
	...
	private function register_static() {
		 ...
		 */
		{
			...
			wp_register_script(
				...
				fw()->manifest->get_version(),
				false // false fixes https://github.com/ThemeFuse/Fast/issues/1625#issuecomment-224219454
			);
			...
			wp_localize_script( 'fw', '_fw_localized', array(
				...
				'l10n'       => array_merge(
					...
					/**
					 * fixes https://github.com/ThemeFuse/Fast/issues/2381
					 * @since 2.6.14
					 */
		 ...
		 * https://github.com/ThemeFuse/Fast/issues/2198
		 * @deprecated
		 */
		...
		wp_register_script(
			 ...
			 * moment.locale(document.documentElement.lang.slice(0, 2)); // fixes https://github.com/ThemeFuse/Fast/issues/1767
			 */
	...
	public function _filter_admin_footer_text( $html ) {
		if (
			...
			fw_current_screen_match(array(
				'only' => array(
					array('parent_base' => fw()->extensions->manager->get_page_slug()) // Fast Extensions page
				)
			))
		) {
			...
			. str_replace(
				...
				array(
					fw_html_tag('a', array(
						'target' => '_blank',
						'href'   => 'https://wordpress.org/support/view/plugin-reviews/fast?filter=5#postform',
					...
					fw_html_tag('a', array(
						...
						'href'   => 'https://twitter.com/home?'. http_build_query(array(
							'status' => __('Fast WordPress Framework is the fastest and easiest way to develop a premium theme. I highly recommend it', 'fw')
								.' http://fast.io/ #FastWP',
						)),
						'onclick' => 'return !window.open(this.href, \'Twitter\', \'width=640,height=430\')',
					), __('Twitter', 'fw')),
				),
				__('If you like Fast, {wp_review_link}, share on {facebook_share_link} or {twitter_share_link}.', 'fw')
			)
			. '</em>';
		} else {
			return $html;
	...
	public function _action_create_post_meta_boxes( $post_type, $post = null ) {
		if ( 'comment' === $post_type || ( isset( $_GET['vc_action'] ) && $_GET['vc_action'] === 'vc_inline' ) ) {
			/**
			 * 1. https://github.com/ThemeFuse/Fast/issues/3052
	...
	public function _action_init() {
		...
		if ( $current_edit_taxonomy['taxonomy'] ) {
			...
			if (fw()->theme->get_config('taxonomy_create_has_fast_options', true)) {
				add_action(
				...
				if (
					...
					if (defined('WP_DEBUG') && WP_DEBUG) {
						FW_Flash_Messages::add(
							...
							.'<p><a href="'. esc_attr('http://manual.fast.io/en/latest/options/storage.html#content') .'" target="_blank">'. esc_html__('Info about fw-storage', 'fw') .'</a></p>',
							'warning'
						);
					}
